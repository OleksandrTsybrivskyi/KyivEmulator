    ...
    while (std::getline(infile, line)) {
        ...
        for (const auto &it: en_instructions) {
            if (it.second == command.substr(0, 2)) {
                ...
                break; // found matching EN instruction, stop iterating
            }
        }

        // If not found in English mnemonics, try Ukrainian mnemonics as a fallback
        if (result.empty()) {
            for (const auto &it: ua_instructions) {
                if (it.second == command.substr(0, 2)) {
                    // For UA mnemonics we don't attempt to create jump labels here;
                    // just emit the mnemonic and raw addresses like for a normal instruction.
                    result.append(it.first + " ");
                    std::string addr_1 = command.substr(2, 4) + " ";
                    std::string addr_2 = command.substr(6, 4) + " ";
                    std::string addr_3 = command.substr(10, 4) + " ";
...
int main(int argc, char *argv[]) {
    ...
    if (mode == "command") {
        ...
        for (const auto& input : memoryInputs) {
            ...
            word_t stored = machine.kmem.read_memory(input.address);
            std::cout << "Cell[" << input.address << "] = " << stored << std::endl;
        }
